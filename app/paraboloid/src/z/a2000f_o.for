...
*           month=mo;						      *
*           day in month=id.					      *
*           ro, s(V), bimf- solar wind density and velocity, IMF.	      *
*           dst - value of Dst index;				      *
*           AL  - value of al index. 				      *
...
*****************************************************************
      ...
      common/fac12/ami1,ami2,tm1,tm2
      real*4 mlt, lat
      DIMENSION X0(3), X0sm(3), par(10),SM2GSM(3,3)
...
c      end if
 ...
 30   CONTINUE
      ...
      open (1, FILE='out/open')
c      if (iprint.eq.1) write (1,123) Y,AB,B
      if ((Y(1)+0.5/R1*(Y(2)**2+Y(3)**2).GT.R1)) then
      write (1,*) x0sm
      else
      continue
      end if
...
c      if (iprint.eq.1) write (1,425) Y
...
c        if (iprint.eq.1) write (1,425) ttcs
...
c        if (iprint.eq.1) write (1,425) teq
